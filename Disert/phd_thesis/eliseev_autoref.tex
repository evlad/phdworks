% -*-coding: cp1251;-*-
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Главный файл
%
% To produce eliseev_phd.dvi run:
%   $ latex eliseev_phd.tex
% To produce eliseev_phd.pdf run:
%   $ pdflatex eliseev_phd.tex
% To produce eliseev_phd.rtf run:
%   $ latex2rtf -i russian -M 12 -S eliseev_phd.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[autoref,subf,href,colorlinks=false
%,facsimile
%,openright
%,fixint=false
%,times
%,classified
]{disser}

\usepackage[
  a4paper, nohead, mag=1000, includefoot,
  margin=2cm, footskip=1cm
]{geometry}

% Standard is 1.5
\renewcommand{\baselinestretch}{1.38}

%  margin=2cm, footskip=1cm

\usepackage[T2A]{fontenc}
\usepackage[cp1251]{inputenc}
\usepackage{rotating}
\usepackage{graphicx}
\usepackage{epstopdf} 
\usepackage{tabularx}

% Now its possible to \includegraphics{file} without extension and
% file.eps or file.pdf will be found depending latex or pdflatex run.
% Moreover, if epstopdf is installed then file.eps is converted to
% file-eps-converted-to.pdf automatically.
%
% If convertion does not work automatically, next command will help:
% epstopdf --outfile=file-eps-converted-to.pdf file.eps
%
% Convert landscape oriented .ps files (created by gnuplot) to .eps
% not by default but using next parameters:
% ps2eps -R + -l -B -f nnp_step_trained.eps nnp_step_trained.ps
\usepackage{latexcad}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{epsfig}
\usepackage{pgf}
\usepackage{tikz}
\usepackage{pstricks}
\usepackage{array}
\usepackage{longtable}
\usepackage{lastpage}

%\usepackage[ps2pdf,bookmarks=true,bookmarksnumbered=true,hypertexnames=false,linkbordercolor={0 0 1},unicode]{hyperref} 

\usepackage[english,russian]{babel}
\ifpdf\usepackage{epstopdf}\fi

\hypersetup{
pdfauthor = {Vladimir Eliseev},
pdftitle = {Vladimir Eliseev - Ph.D. thesis autoreferate},
pdfsubject = {Development and research of neural network control
  algorithms for stationary and non-stationary objects},
pdfkeywords = {neural network, automated control, optimal control,
  control system},
pdfcreator = {LaTeX with hyperref package},
pdfproducer = {pdflatex}}
\language=4

%\bibliographystyle{plain}
%  $ bibtex eliseev_phd.aux

% disser package is needed to get sorted bibliography
%  $ bibtex8 -H -c cp1251 eliseev_phd.aux
\bibliographystyle{gost705s}

% My macros
\newcommand\tablref[1]{\cyrt\cyra\cyrb\cyrl.~\ref{#1}}
\newcommand\figref[1]{\cyrr\cyri\cyrs.~\ref{#1}}
\newcommand\pgref[1]{\cyrs.~\pageref{#1}}
%\newcommand\eqref[1]{(\ref{#1})}

\renewcommand{\tan}{\mathop{\rm tg}}
\renewcommand{\arctan}{\mathop{\rm arctg}}
\renewcommand{\tanh}{\mathop{\rm th}}
\newcommand{\DEF}{\mathrel{\mathop=^{\rm def}}}
\newcommand{\MSE}{\overline{\varepsilon^2}}
\newcommand{\hatMSE}{\overline{\hat{\varepsilon}^2}}
\newcommand{\MSEmin}{\overline{\varepsilon^2}_{\rm min}}
\newcommand{\dt}{\Delta t}
\newcommand{\NN}{\mathcal{N}}
\newcommand{\Emax}{|e|_{max}}

% For disser:
\newcommand{\bbljan}{\cyrya\cyrn\cyrv\cyra\cyrr\cyrsftsn}
\newcommand{\bblfeb}{\cyrf\cyre\cyrv\cyrr\cyra\cyrl\cyrsftsn}
\newcommand{\bblmar}{\cyrm\cyra\cyrr\cyrt}
\newcommand{\bblapr}{\cyra\cyrp\cyrr\cyre\cyrl\cyrsftsn}
\newcommand{\bblmay}{\cyrm\cyra\cyrishrt}
\newcommand{\bbljun}{\cyri\cyryu\cyrn\cyrsftsn}
\newcommand{\bbljul}{\cyri\cyryu\cyrl\cyrsftsn}
\newcommand{\bblaug}{\cyra\cyrv\cyrg\cyru\cyrs\cyrt}
\newcommand{\bblsep}{\cyrs\cyre\cyrn\cyrt\cyrya\cyrb\cyrr\cyrsftsn}
\newcommand{\bbloct}{\cyro\cyrk\cyrt\cyrya\cyrb\cyrr\cyrsftsn}
\newcommand{\bblnov}{\cyrn\cyro\cyrya\cyrb\cyrr\cyrsftsn}
\newcommand{\bbldec}{\cyrd\cyre\cyrk\cyra\cyrb\cyrr\cyrsftsn}

% Activation function
\newcommand{\fa}{\phi}

% Gauss Distribution parameters
\newcommand{\GaDi}[2]{$(#1;#2)$}

% Позволить переносы двух последних букв слова
\righthyphenmin=2

% Дополнительные переносы
\hyphenation{ошиб-ки обыч-но сис-тем не-по-сред-ствен-но-го
ус-пеш-но раз-бро-са раз-брос ав-то-рег-рес-сии од-но-вре-мен-но
не-чет-ко-ло-ги-че-ский на-уч-но-ис-сле-до-ва-тель-ских
back-pro-pa-ga-tion out-put}

\input{txdtools.tex}
\input{blockdiagram.tex}

% Для макросов texdraw установить умолчательной единицей измерения миллиметры
%texdraw \everytexdraw{\drawdim mm}

\begin{document}

\input{common_defines.tex}

\title{АВТОРЕФЕРАТ\\
диссертации на соискание ученой степени\\
кандидата технических наук}

% Титульные страницы
\maketitle

% Внутренняя сторона обложки
\noindent
\begin{center}
Работа выполнена в федеральном государственном бюджетном
образовательном учреждении высшего профессионального образования
``Национальный исследовательский университет ``МЭИ''.
\end{center}
\vskip1ex
\begin{tabularx}{\linewidth}{lp{1cm}X}
Научный руководитель:  & & доктор технических наук,\\
                       & & профессор,\\
                       & & Филаретов Геннадий Федорович\\[3pt]
Официальные оппоненты: & & доктор технических наук,\\
                       & & профессор,\\
                       & & Лохин Валерий Михайлович\\
                       & & доктор технических наук,\\
                       & & профессор,\\
                       & & Ермуратский Петр Васильевич\\[3pt]
Ведущая организация:   & & ОАО ``Газпром автоматизация''\\
\end{tabularx}

\vskip5ex\noindent
Защита состоится 26 апреля 2012 г. в 17 часов на заседании
диссертационного совета Д 212.157.08 при ФГБОУ ВПО ``НИУ МЭИ'' в малом
актовом зале, расположенном по адресу: 111250, Москва,
Красноказарменная улица, д.14.

\vskip1ex\noindent
С диссертацией можно ознакомиться в библиотеке ФГБОУ ВПО ``НИУ МЭИ''.

\vskip1ex\noindent
Автореферат разослан \datefield[2012]{}

\vskip2ex\noindent
Отзывы и замечания по автореферату в двух экземплярах, заверенные
печатью, просьба высылать по вышеуказанному адресу на имя ученого секретаря
диссертационного совета.

\vfill\noindent
Ученый секретарь\\
диссертационного совета,\\
кандидат технических наук, доцент%
\hfill
\makeatletter
% вставка файла, содержащего факсимиле ученого секретаря
%\ifDis@facsimile
%  \raisebox{-4pt}{\includegraphics[width=3cm]{sec-facsimile}}\hfill
%\fi%
\makeatother%
Анисимов Д. Н.

\thispagestyle{empty}
\clearpage

\setcounter{page}{1}
\nsection{Введение}

%%                          ПРОВЕРИТЬ!
\newcommand{\NumberOfPages}{208}
\input{common_intro.tex}


\paragraph{Публикации}
По результатам исследований опубликовано 13 научных работ, в том
числе, 2 публикации в рецензируемых журналах, входящих в список ВАК
РФ.

\paragraph{Структура и объем работы}
%\marginpar{ПРОВЕРИТЬ!}
Диссертационная работа состоит из введения, шести глав, заключения и
списка литературы из 85 наименований, включает \NumberOfPages\ страниц
текста, 75 рисунков, 14 таблиц.

% Take care of autoreferat!!!
% Number of figures may be counted so:
%  $ grep 'label[{]fig:' *.tex | wc -l
% Number of tables may be counted so:
%  $ grep 'label[{]tabl:' *.tex | wc -l

\nsection{Содержание работы}

% Собственно, автореферат
\input{autoref.tex}

\nsection{Основные результаты}
\input{conclusion.tex}

% Библиография
%\bibliographystyle{gost705s}
\renewcommand\bibsection{\nsection{Публикации по теме диссертации}}
%\bibliography{mypapers}
\input{eliseev_autoref.bbl}

\end{document}
