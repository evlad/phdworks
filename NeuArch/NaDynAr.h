...
public:
  ...
  /* Add an item at the end of array (addh) or at the
     beginning of array (addl).  Return its index. */
  ...
  /* Add an item at the end of array (addh) or at the
     beginning of array (addl).  Return its index.  Create
     new instance of an object by means of copying
     constructor. */
  ...
  /* Add array of items making new for each one.  Create
...
/* Create copy of the array, meaning there is a type(type&)
   constructor. */
template <class type> inline
NaDynAr<type>::NaDynAr (const NaDynAr& dynar)
{
  pTail = pHead = NULL;
  nItems = 0;
  int	i;
...
/* Assign one array to another. */
template <class type> inline NaDynAr<type>&
NaDynAr<type>::operator= (const NaDynAr<type>& dynar)
{
...
NaDynAr<type>::print_contents () const
{
  int	i = 0;
...
template <class type> inline
NaDynAr<type>::item::item (type* ptr)
{
  pData = ptr;
  pNext = NULL;
}
...
/* Print self contents */
template <class type> inline void
NaDynAr<type>::item::print_self () const
{
...
/* Insert an item into the array just before given index.
   Return its index. */
template <class type> inline int
NaDynAr<type>::insert (int index, type* pobj)
{
  if(NULL == pobj)
    throw(na_null_pointer);
...
/* Add an item at the end of array.  Return its index.
   ...
   constructor. */
template <class type> inline int
NaDynAr<type>::addh (const type& obj)
{
...
/* Add an item at the beginning of array.  Return its index.
   ...
   constructor. */
template <class type> inline int
NaDynAr<type>::addl (const type& obj)
{
  type	*pobj = new type(obj);
  return addl(pobj);
}
...
/* Insert an item into the array just before given index.
   ...
   means of copying constructor. */
template <class type> inline int
NaDynAr<type>::insert (int index, const type& obj)
{
  type	*pobj = new type(obj);
...
/* Add array of items making new for each one.  Create
   new instance of an object by means of copying constructor. */
template <class type> inline void
NaDynAr<type>::addh (const NaDynAr<type>& arr)
{
    int     i;
...
/* Add array of items making new for each one.  Create
   new instance of an object by means of copying constructor. */
template <class type> inline void
NaDynAr<type>::addl (const NaDynAr<type>& arr)
{
    int     i;
    for(i = 0; i < arr.count(); ++i){
...
/* Insert array of items making new for each one.  Create
   new instance of an object by means of copying constructor. */
template <class type> inline void
NaDynAr<type>::insert (int index, const NaDynAr<type>& arr)
{
    int     i;
    for(i = 0; i < arr.count(); ++i){
...
template <class type> inline type&
NaDynAr<type>::operator[] (int index)
{
  return fetch(index);
}
...
NaDynAr<type>::get (int index) const
{
  item	*p = fetch_item(index);
...
template <class type> inline const type&
NaDynAr<type>::operator() (int index) const
{
  return get(index);
}
...
NaDynAr<type>::good (int index) const
{
